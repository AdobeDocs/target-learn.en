...
---
# Set up A4T reports in [!DNL Analysis Workspace] for [!DNL Auto-Allocate] activities 

An [[!UICONTROL Auto-Allocate] activity](https://experienceleague.adobe.com/docs/target/using/activities/auto-allocate/automated-traffic-allocation.html){target=_blank} in [!DNL Adobe Target] identifies a winner among two or more experiences and automatically reallocates visitor traffic to the winner while the test continues to run and learn. The [!UICONTROL Analytics for Target] (A4T) integration for [!UICONTROL Auto-Allocate] lets you view reporting data in [!DNL Adobe Analytics], and you can optimize for custom events or metrics defined in [!DNL Analytics].
...
| --- | --- | --- |
|Maximize metric value for an [!DNL Analytics] metric|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove [!UICONTROL Lift (Low)] and [!UICONTROL Lift (High)]. Keep [!UICONTROL Lift (Med)].</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li><li>Rename the [!UICONTROL Conversion] rate metric to "Metric / Visitor."</li></ul>|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove [!UICONTROL Lift (Low)] and [!UICONTROL Lift (High)] Keep [!UICONTROL Lift (Med)].</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li><li>Rename the [!UICONTROL Conversion] rate metric to "Metric / Visitor."</li><li>Ensure that the date and time ranges align with the values you see in the [!DNL Target] report. See [Overall guidance for A4T](#guidance) below.</li></ul>|
...
| --- | --- | --- |
|Maximize conversions for an [!DNL Analytics] metric|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove all three [!UICONTROL Lift] metrics.</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li></ul>|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove all three [!UICONTROL Lift] metrics.</li><li>Create a segment to filter visitors with a positive metric value who viewed the activity that was analyzed. See [Create a segment](#segment) below.</li><li>Replace the auto-populated [!UICONTROL Conversion Rate] metric so that is the division between [!UICONTROL Unique visitors] with a positive metric value and unique visitors. See [Update the Conversion Rate metric](#update-conversion-metric) below.</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li><li>Ensure that the date and time ranges align with the values you see in the [!DNL Target] report. See [Overall guidance for A4T](#guidance) below.</li></ul>|
...
1. Drag and drop your newly created "Conversion Rate" metric into your existing panel. 
...
|Changes required|Target-triggered report|A4T Panel report|
| --- | --- | --- |
|[!DNL Analytics] reporting with [!DNL Target] conversion metric|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove [!UICONTROL Lift (Low)] and [!UICONTROL Lift (High)]. Keep Lift (Med).</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li></ul>|<ul><li>Remove [!UICONTROL Confidence] metrics.</li><li>Remove [!UICONTROL Lift (Low)] and [!UICONTROL Lift (High)]. Keep [!UICONTROL Lift (Med)].</li><li>Uncheck the percentage presentation from the [!UICONTROL Conversion Rate] column to avoid confusion. See [Overall guidance for A4T](#guidance) below.</li><li>Ensure that the date and time ranges align with the values you see in the [!DNL Target] report. See [Overall guidance for A4T](#guidance) below.</li></ul>|
...
You can navigate to a pre-built [!UICONTROL Analytics for Target] panel by clicking the link from the report screen in [!UICONTROL Target] (this is referred to later in this guide as the "[!DNL Target]-triggered report"). Alternatively, you can build the A4T panel in [!DNL Analytics] (details later in this section). 
...
1. Deselect the **[!UICONTROL Percent]** checkbox.
   ...
   Your A4T panel now does not include percentages as the [!UICONTROL Conversion Rate] and matches [!DNL Target], as shown below:
